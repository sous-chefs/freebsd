...
when /10/
  ...
  s = script 'create non-interactive portsnap' do
    ...
    action(node['freebsd']['compiletime'] ? :nothing : :run)
  end
  s.run_action(:run) if node['freebsd']['compiletime']
end

# Ensure we have a ports tree
unless File.exist?('/usr/ports/.portsnap.INDEX')
  e = execute "#{portsnap_bin} fetch extract #{portsnap_options}" do
    action(node['freebsd']['compiletime'] ? :nothing : :run)
  end
  e.run_action(:run) if node['freebsd']['compiletime']
end

e = execute "#{portsnap_bin} update #{portsnap_options}" do
  action(node['freebsd']['compiletime'] ? :nothing : :run)
end
e.run_action(:run) if node['freebsd']['compiletime']
